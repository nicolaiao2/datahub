...
export abstract class SourcesBaseTab extends BaseTab {
  ...
  async confirmExecution(): Promise<void> {
    ...
    // Scope to the named dialog — AntD portal modals accumulate in DOM across retries,
    // making a bare modal-confirm-button locator resolve to multiple visible elements.
    const dialog = this.page.getByRole('dialog', { name: 'Confirm Source Execution' });
  ...
  async deleteIngestionSource(sourceName: string): Promise<void> {
    ...
    const dialog = this.page.getByRole('dialog', { name: 'Confirm Ingestion Source Removal' });
    await dialog.waitFor({ state: 'visible' });
    await dialog.locator('[data-testid="modal-confirm-button"]').click();
