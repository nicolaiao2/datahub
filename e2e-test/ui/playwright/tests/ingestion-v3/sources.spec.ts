...
// Pre-seeded via fixtures/data.json — no create/delete overhead in tests that use them.
...
test.describe('ingestion sources', () => {
  ...
  test('create, edit, and delete an ingestion source', async ({ page }) => {
    ...
    await ingestionPage.sourcesTab.deleteIngestionSource(updatedSourceName);
    await expect(page.getByText(updatedSourceName)).toBeHidden();
  });

  test('create and run an ingestion source', async () => {
    test.slow();

    const sourceName = `create and run source ${randomSuffix()}`;
  ...
  test('execute an ingestion source', async () => {
    ...
    await ingestionPage.sourcesTab.runIngestionSource(EXECUTE_SOURCE);
